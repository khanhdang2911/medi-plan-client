	...
	return (
		<Box>
			<Box sx={{ display: 'flex', justifyContent: 'center', bgcolor: '#efefef' }}>
				<Box sx={{ width: (theme) => theme.booking_care.WIDTH_COMMON, display: 'flex', justifyContent: 'space-between', padding: '20px 0' }}>
					<Box sx={{ width: '467px' }}>
...
function ContactLink({ label, to }) {
	return (
		<Link to={to}>
			<Typography sx={{ color: 'rgb(69, 195, 210)', fontSize: '14px', fontWeight: '600', padding: '3px 0' }}>{label}</Typography>
		</Link>
	)
}
